...
function getJamPraktek($tanggal, $poli){
        $tgl_ubah = date("Y-m-d", strtotime($tanggal));
        
        $time = mysql_query("select * from inbox where Processed = 'false' order by ReceivingDateTime desc");
		$waktu = mysql_fetch_array($time);
		$receivetime = $waktu['ReceivingDateTime'];			
		...
		a.jam_praktek > '$times' AND a.tanggal_praktek = '$tgl_ubah' AND b.nama_pelayanan = '$poli' order by a.pk ASC LIMIT 1");
...
function CekPembayaran($pembayaran) {
    $query = mysql_query("SELECT * FROM m_metode_pembayaran WHERE metode_bayar = '$pembayaran'");
...
function getidsession($tgl_ubah, $poli){
	$idsession = mysql_query("SELECT id_session_jadwal from m_jadwal_praktek as a join m_pelayanan as b on a.pelayanan_fk = b.pk 
							where a.tanggal_praktek = '$tgl_ubah' and b.nama_pelayanan = '$poli'");	
...
function add_antrian($noreg, $jamplus1, $id, $pembayaran){
	...
	$query4 = mysql_query("SELECT * from m_jadwal_praktek_antrian as a join m_jadwal_praktek as b on a.jadwal_praktek_fk = b.pk 
							where b.id_session_jadwal = '$id' order by a.nomor_urut desc");
...
function balasan($noreg, $id, $poli){
	$pasien = mysql_query("select * from m_kontak as a join m_jadwal_praktek_antrian as b on a.pk = b.pasien_fk where a.status_login = 'pasien' 
							and a.nomor_registrasi = '$noreg'");			
	...
	if($m_pk < $m__pk){
		$reply = "$m_nama - $m_noreg - $m_tanggal - $m_jam - No Urut : $m_antrian - $m_dokter - SELAMAT ANDA TELAH TERDAFTAR SEBAGAI PASIEN $poli 
				DI RUMAH SAKIT AMAL SEHAT";
	}else{
		$reply = "$m_nama - $m_noreg - $m_tanggal - $m_jam - No Urut : $m_antrian - $m_dokter - USERNAME : $m_username - PASSWORD : $m_password - SELAMAT ANDA TELAH 
					TERDAFTAR SEBAGAI PASIEN $poli DI RUMAH SAKIT AMAL SEHAT";
	} 
	return $reply;
}
