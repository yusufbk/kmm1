...
function getJamPraktek($tanggal, $poli){
    ...
    $time = mysql_query("select * from sms_inbox where Processed = 'false' order by waktu_terima desc");
...
function balasan($noreg, $id, $poli){
	...
	if($m_pk < $m__pk){
		...
		$reply = "$m_nama - $m_noreg - $m_tanggal - $m_jam - No Urut : $m_antrian - $m_dokter - USERNAME : $m_username - PASSWORD : $m_password - SELAMAT ANDA 
		TELAH TERDAFTAR SEBAGAI PASIEN $poli DI RUMAH SAKIT AMAL SEHAT";
	} 
	return $reply;
}
