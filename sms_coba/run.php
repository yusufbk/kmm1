 ...
 if($command == "INFO POLI"){
	...
	if ($jenkel == "L" || $jenkel == "P"){	
        if (CekPembayaran($pembayaran) != NULL){
			if ($tgl_pesan == $today2){
                //hari ini
				$jamplus1 = getJamPraktek($tanggal, $poli);
				if($jamplus1 != NULL){										
             ...
             $reply = "Tanggal yang ada masukkan salah, pastikan format tanggal benar (hari-bulan-tahun) atau cek kembali tanggal anda";
        }else 
        $reply = "Metode pembayaran '$pembayaran' tidak tersedia, coba metode lain atau hubungi pihak rumah sakit.";
        }else 
        $reply = "Jenis Kelamin '$jenkel' tidak tidak diketahui, pastikan jenis kelamin L atau P.";
    }else
